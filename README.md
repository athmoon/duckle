...
- **Oracle extracts are about twice as fast (#221).** `src.oracle` used to format every value to JSON text, write it to disk, and have DuckDB parse it back. It now streams Arrow batches straight into Parquet, which measured 21.1s -> 10.1s on 1M rows x 40 columns against a real Oracle 23 instance, with values verified identical. Where the fast path applies, types also improve: a `NUMBER(15,2)` lands as `DECIMAL(15,2)` instead of `DOUBLE`, so values beyond ~15 significant digits stop silently losing their tail. Columns whose Oracle type does not pin an exact mapping (unconstrained `NUMBER`, LOBs, timestamps carrying a zone) keep the previous path and previous behaviour.
